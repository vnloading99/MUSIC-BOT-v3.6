...
return interaction?.reply({ content: "Giới Hạn Tần Suất Hoạt Động.", ephemeral: true })
} else {
    ...
    function cmd_loader() {
        if (interaction?.type === InteractionType.ApplicationCommand) {
            ...
            return interaction?.reply({ content: '🔴 Giới Hạn Tần Suất Hoạt Động'.replace("{channel_filter}", channel_filter), ephemeral: true }).catch(e => { })
            }
        }
            }
            ...
            if (!interaction?.member?.voice?.channelId) return interaction?.reply({ content: `🔴 Hãy tham gia kênh Voice trước!!`, ephemeral: true }).catch(e => { })
            ...
            return interaction?.reply({ content: `▶️ Thiếu quyền hạn: **${props?.permissions?.replace("0x0000000000000020", "MANAGE GUILD")?.replace("0x0000000000000800", "SEND MESSAGES") || "SEND MESSAGES"}**`, ephemeral: true });
            }
            } catch (e) {
